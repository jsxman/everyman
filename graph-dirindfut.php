...
$count=0;
if($row4['access_sw']) {$count++;}
if($row4['access_se']) {$count++;}
if($row4['access_de']) {$count++;}
if($row4['access_ts']) {$count++;}
if($row4['access_en']) {$count++;}
if($row4['access_eo']) {$count++;}
if($count<6)
{
...
$strSQL1.= ",PR.future, PR.headcount_scaling";
...
while($row1 = mysql_fetch_array($result1))
{
   ...
   if($row1['future']==1)
   {
     $project=1;// $project='Future';
     $names[$project]="Future";
     if(!isset($data0[$project][$count]))$data0[$project][$count]=0;
     $data0[$project][$count]+= $row1['thours'] * $row1['headcount_scaling'] / 100;
   }
   else
   {
     $project=2;// $project='Direct';
     $names[$project]="Direct";
     if(!isset($data0[$project][$count]))$data0[$project][$count]=0;
     $data0[$project][$count]+= $row1['thours'] * $row1['percent_direct'] / 100;

     $project=3;// $project='inDirect';
     $names[$project]="Indirect";
     if(!isset($data0[$project][$count]))$data0[$project][$count]=0;
...
$x=$strAccum?" Accum":"";
$textBoxObj = $c->addTitle("Eng Dept: Staff Level by Project (Future Scaled by P-Win)", "timesbi.ttf", 12);
unset($x);
...
$dataX=Array();
for($i=0;$i<12;$i++){$dataX[$i]=$TOTAL_STAFF;}
...
//$TB = $layer->setAggregateLabelStyle("timesbi.ttf", 10, 0x000000, 30);
