...
while($row1 = mysql_fetch_array($result1))
{
   ...
   if($row1['future']==1)
   {
     ...
     $names[$project]="Future (>60%)";
     if(!isset($data0[$project][$count]))$data0[$project][$count]=0;
     $scale= $row1['headcount_scaling'];
     if($scale<$SCALE_LIMIT){$project=4;$names[$project]="Future (<60%)";}
     $data0[$project][$count]+= $row1['thours'];
   }
   else
   {
...
$x=$strAccum?" Accum":"";
$textBoxObj = $c->addTitle("Eng. Dept. Staff Level by Project (Future Split - Not Scaled)", "timesbi.ttf", 12);
unset($x);
