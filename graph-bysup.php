...
$strAccum = 0;
$strPerson =isset($_FORM['txtperson'])?postedData($_FORM['txtperson']):0;
...
$strSQL0 = "SELECT";
$strSQL0.= " AP.date, sum(AP.hours) as thours, S.lname, S.fname";
$strSQL0.= " FROM $TABLE_ACTIVEPERSON AS AP";
$strSQL0.= " LEFT JOIN $TABLE_PERSON AS P ON P.id=AP.person_id";
$strSQL0.= " LEFT JOIN $TABLE_PERSON AS S ON S.id='$strPerson'";
$strSQL0.= " WHERE AP.date >= '$sYear-$sMonth-01' AND AP.date <= '$eYear-$eMonth-01' AND P.supervisor_id='$strPerson'";
...
//$MAX_HOURS=Array();
$max=0;
$strSQL2 = "SELECT lname, fname FROM $TABLE_PERSON WHERE id=$strPerson";
$result2 = dbquery($strSQL2);
$row2 = mysql_fetch_array($result2);
$supName=$row2['lname'].",".$row2['fname'];

while($row0 = mysql_fetch_array($result0))
{
  $supName=$row0['lname'].", ".$row0['fname'];
...
$strSQL1 = "SELECT";
$strSQL1.= " AP.date, sum(AP.hours) as thours";
$strSQL1.= " FROM $TABLE_ACTIVEPERSON AS AP";
$strSQL1.= " LEFT JOIN $TABLE_PERSON AS P ON P.id=AP.person_id";
$strSQL1.= " WHERE AP.date >= '$sYear-$sMonth-01' AND AP.date <= '$eYear-$eMonth-01' AND P.supervisor_id='$strPerson'";
$strSQL1.= " GROUP BY AP.date";
...
$x=$strAccum?" Accum":"";
$textBoxObj = $c->addTitle("Staff Level: $supName", "timesbi.ttf", 12);
unset($x);
